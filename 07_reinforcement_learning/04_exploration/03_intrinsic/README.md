...
⬅️ [Back: Epsilon-Greedy](../02_epsilon_greedy/) | ➡️ [Next: UCB](../04_ucb/)
...
Theorem (UCB Regret Bound):
  For UCB exploration with bonus c√(log t / N(s,a)):
  
  Cumulative regret ≤ O(√(|S||A|T log T))
  
  After T steps, regret grows sublinearly → optimal!
...
Intrinsic reward = information gain about dynamics:
  ...
  r_i(s,a,s') = D_KL[p(θ|D ∪ {s,a,s'}) || p(θ|D)]

Where:
  θ = dynamics model parameters
  D = replay buffer
...
Solution: Normalize by running statistics:
  r_i^norm = (r_i - μ_i) / σ_i

Where μ_i, σ_i are running mean/std of intrinsic rewards.
...
```
Adaptive β (intrinsic coefficient):
  β_t = β_0 · decay^t
